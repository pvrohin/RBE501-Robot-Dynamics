...
for jj = 1 : nPts - 1
    ...
    for ii = 1 : size(t,2) - 1
        ...
        T = fkine2(S,M,jointPos_prescribed(:,ii));
        adj = adjoint(T);
        params_rne.Ftip = adj'*[cross(T(1:3,4),-g) -g]';%zeros(6,1); % end effector wrench [0 0 9.81 0 0 0]
        ...
        %joint actual
        T = fkine2(S,M,jointPos_actual(:,ii));
        adj = adjoint(T);
        params_fdyn.Ftip = adj'*[cross(T(1:3,4),-g) -g]';
        %params_fdyn.Ftip = [0 0 0 0 0 9.81]';%zeros(6,1); % end effector wrench [0 0 9.81 0 0 0]
